                               ...
                               USBD_SetupReqTypedef *req)
{
  ...
  switch (req->bmRequest & USB_REQ_TYPE_MASK)
  {
      ...
      switch (req->bRequest)
      {
        ...
        case HID_REQ_SET_REPORT:
          if (req->wLength == 1)
          {
            ...
            USBD_CtlPrepareRx(pdev, hhid->Report_buf, 1);
            break;
          }
          /* длина репорта клавиатуры == 1, иначе - ошибка */
    ...
    case USB_REQ_TYPE_VENDOR:
      if (req->bRequest == VENDOR_REQ_SET_PASSWORD_CHAR)
      {
        /* установка символа нового пароля */
        int index = req->wValue & 0xff;
        uint8_t ch = (req->wValue >> 8) & 0xff;
        setNewPasswordChar(ch, index);
...
static uint8_t USBD_HID_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  {
    HID_KBD_GetReport(hhid->Report_buf);
